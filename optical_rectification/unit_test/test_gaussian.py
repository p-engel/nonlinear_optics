...
def test_gaussian():
    try:
        pulse = definitions.Gaussian(w0=w0, tau=tau, E0=E0)
        E_w = pulse.field_w(w)
        ind_cond = abs(1/2 - E_w/pulse.E0_w) < 1e-1
        fwhm = abs( max(w[ind_cond]) - min(w[ind_cond]) )
        print("The bandwith (FWHM) is ", fwhm, " [THz]." )
        cond = abs( fwhm*tau - definitions.tau_delta ) < 1e-1
